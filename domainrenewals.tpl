			...
			<table class="table">
				...
				<tr>
					...
					<td>
						...
						<span class="text-error">{$renewal.daysuntilexpiry} {$LANG.domainrenewalsdays}</span>
						{else}
						{$renewal.daysuntilexpiry*-1} {$LANG.domainrenewalsdaysago}
						{/if}
						{if $renewal.ingraceperiod}
						<br>
						<span class="text-error">{$LANG.domainrenewalsingraceperiod}</span>
						{/if}
					</td>
					<td>
						{if $renewal.beforerenewlimit}
						<span class="text-error">{$LANG.domainrenewalsbeforerenewlimit|sprintf2:$renewal.beforerenewlimitdays}</span>
						{elseif $renewal.pastgraceperiod}
						<span class="text-error">{$LANG.domainrenewalspastgraceperiod}</span>
						{else}
